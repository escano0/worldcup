...
- **#4 `parse_score` 未走生产路径**:行解析直接用 `_ROW_RE` 的比分捕获组取整数,`parse_score` 仅作为独立、已测试的工具函数保留(供将来复用),非死代码但当前 CLI 链路不调用。
- **行解析健壮性已加固**(评审 #1/#2):赛事名捕获用 `(\D+?)`(吸收多词赛事名但不跨数字,避免把尾部赛季/伤病区的日期误判成比分行);并加 `played == len(recent)` 对账,任何静默丢行都会 `raise ValueError` 暴露出来。
