...
| 价值投注推荐(模型 vs 盘口的 +EV + 凯利注码,**实验性/未回测**) | `docs/recommendations.json` |
...
PYTHONPATH=src python -m worldcup.recommend --out docs/recommendations.json  # 价值投注(纯本地,读 odds+teams)
```

> **关于 `recommendations.json`(agent 引用须知)**:简化 Dixon-Coles + 价值投注的**实验性**输出,**未回测**、信号偏多偏噪声。引用时务必说明"仅供参考、非稳赚、博彩有风险",不要当作可靠投注建议。
